...
//  Stocks.EduProj
//
//  Created by Виктор on 31.03.2021.
//
...
struct LineView: View {
    ...
    public var body: some View {
        GeometryReader { geometry in
            Line(data: self.data,
                 frame: .constant(CGRect(x: 0, y: 0, width: geometry.frame(in: .local).width , height: geometry.frame(in: .local).height))
            )
        }
    }
}
...
//                print((frame.size.height-padding) / CGFloat(max + min)*10)
                return (frame.size.height) / CGFloat(max + min) * 200
            }
        }
        return 0
    }
